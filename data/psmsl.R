...
## Make a station for AMSTERDAM/DEN HELDER
station.helderdam <- list()
station.helderdam$id <- max(as.numeric(names(stations)))+1
station.helderdam$latitude <- 52.598675
station.helderdam$longitude <- 5.025455
station.helderdam$name <- "HELDERDAM"
station.helderdam$coastline.code <- 150
station.helderdam$station.code <- 10001
station.helderdam$quality.flag <- "N"
station.helderdam$data <- read.table("helderdam.csv", sep=",", header=T)
station.helderdam$data$days.missing <- "N"
station.helderdam$data$flag <- 0
station.helderdam$peltier <-  mean(laply(stations.dutch, function(station){station$peltier}))
stations[[as.character(station.helderdam$id)]] <- station.helderdam
